...
fn main() {
    ...
    {
        // Выделить память для еще одного целого число в куче
        let _box3 = Box::new(4i32);

        // `_box3` здесь уничтожается, а память освобождается
    }

    // Создаем большое количество упаковок. Просто потому что можем.
    // Здесь нет необходимости освобождаеть память вручную!
    for _ in 0u32..1_000 {
        create_box();
    }
